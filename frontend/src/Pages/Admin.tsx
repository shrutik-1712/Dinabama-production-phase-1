...
type TabData = "advisory" | "team" | "alumni" | "events" | "books";
...
const AdminPanel = () => {
  ...
  const [currentItem, setCurrentItem] = useState<
    Member | TeamMember | Alumni | Event | null
  >(null);
  ...
  const [activeTab, setActiveTab] = useState<TabData>("advisory");
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const [advisoryResponse, teamResponse, alumniResponse, eventsResponse] =
        await Promise.all([
      ...
      const [advisoryData, teamData, alumniData, eventsData] =
        await Promise.all([
  ...
  const handleEdit = (item: Member | TeamMember | Alumni | Event) => {
    ...
    if ("title" in item) {
      ...
      setFormData({
        ...
        posting: "posting" in item ? item.posting : "",
        Rank: "Rank" in item ? item.Rank : "",
        image: null,
      });
    }
    setIsModalOpen(true);
  ...
  const getItemId = (item: Member | TeamMember | Alumni | Event): string => {
    if ("title" in item) {
      return item._id;
    }
    return item.name;
  ...
  const handleDelete = async (item: Member | TeamMember | Alumni | Event) => {
    try {
      let endpoint;
      switch (activeTab) {
        case "advisory":
  ...
  const handleDeleteEventImage = async (eventId: string, filename: string) => {
    try {
      const response = await fetch(
        `${API_URL}/events/${eventId}/images/${filename}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (activeTab === "events") {
      ...
      if (activeTab === "team") {
        ...
        formDataToSend.append("posting", formData.posting);
      }
      if (formData.image) {
    ...
    try {
      ...
      const url = currentItem
        ? `${API_URL}/${endpoint}/${encodeURIComponent(getItemId(currentItem))}`
        : `${API_URL}/${endpoint}`;
      ...
      toast.success(
        `${activeTab} item ${currentItem ? "updated" : "created"} successfully`
      );
  ...
  return (
    <div className="p-4">
      ...
      <Tabs
        value={activeTab}
        onValueChange={(value) => setActiveTab(value as TabData)}
      >
        <TabsList>
            ...
            <TableBody>
              {alumniData.map((item) => (
                <TableRow key={item._id}>
                  ...
                  <TableCell>{item.posting}</TableCell>
                  <TableCell>
                    <img
                      src={`http://localhost:3001${item.image}`}
                      alt={item.name}
